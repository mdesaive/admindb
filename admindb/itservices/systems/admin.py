...
from itservices.systems.models import GenericSystem
from itservices.systems.models import GenericVirtualSystem
...
class GenericVirtualSystemInline(admin.TabularInline):
    model = GenericVirtualSystem
    extra = 1
    exclude = ('note', )

    fk_name = 'host'
    readonly_fields = ['genericsystem_ptr']

#
# GenericVirtualSystem with its children.
#
class GenericVirtualSystemChildAdmin(PolymorphicChildModelAdmin):
    base_model = GenericVirtualSystem
    fieldsets = [(
        None, {
...
class ContainerAdmin(GenericVirtualSystemChildAdmin):
...
class VMAdmin(GenericVirtualSystemChildAdmin):
    base_model = VM
...
# class GenericVirtualSystemAdmin(GenericSystemAdmin):
#     pass


class GenericVirtualSystemParentAdmin(PolymorphicParentModelAdmin):
    base_model = GenericVirtualSystem
    child_models = (
...
#
# GenericSystem with its children.
#
class GenericSystemChildAdmin(PolymorphicChildModelAdmin):
    base_model = GenericSystem
    fieldsets = [(
        None, {
            'fields': ['name', 'description', 'note', 'itservice', 'landspace']}),
    ]
    inlines = [GenericVirtualSystemInline]


class ComputerAdmin(GenericSystemChildAdmin):
    base_model = Computer
    # inlines = (ContainerInline, VMInline, ImportSystemsAggregatedInline)
    # list_display = ('name', 'description', 'itservice', 'landspace')
    pass
...
class ClusterAdmin(GenericSystemChildAdmin):
    base_model = Cluster
    # inlines = (ClusterMapComputerInline, ContainerInline, VMInline, ImportSystemsAggregatedInline)
    # list_display = ('name', 'cluster_technology',
    #                 'description', 'itservice', 'landspace')
...
class GenericSystemParentAdmin(PolymorphicParentModelAdmin):
    base_model = GenericSystem
    child_models = (
        (GenericVirtualSystem, GenericVirtualSystemChildAdmin),
        (Computer, ComputerAdmin),
        (Cluster, ClusterAdmin),
    )

#
# Non-polymorphic admins.
#

class ImportSystemsAdmin(admin.ModelAdmin):
    list_display = ('name', 'check1', 'check2', 'itservice', 'group', 'landspace', 'tag1', 'tag2', 'tag3')
    ordering = ('group', 'name')

admin.site.register(Landspace)
admin.site.register(GenericSystem, GenericSystemParentAdmin)
admin.site.register(GenericVirtualSystem, GenericVirtualSystemParentAdmin)
admin.site.register(VirtualizationTechnology)
...
admin.site.register(ImportSystems, ImportSystemsAdmin)
